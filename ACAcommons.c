...
uint32_t PI_control(uint16_t pv, uint16_t setpoint, uint8_t uint_PWM_Enable) {
	...
	if (float_p + float_i > float_dc + 5) { /// was 5
		float_dc += 5;
...
void updateRequestedTorque(void) {
	...
	//#1_val_from_UART Original string is up^^^^^
	ui16_momentary_throttle = (uint16_t) map(ui8_value, ADC_THROTTLE_MIN_VALUE, ADC_THROTTLE_MAX_VALUE, 0, SETPOINT_MAX_VALUE); //read in recent throttle value for throttle override
	...
	if (((ui16_aca_flags & TQ_SENSOR_MODE) != TQ_SENSOR_MODE)) {
		...
		//#1_val_from_UART
		//printf("updatereqestTrque: #1 tqsensor mode  \r\n"); ///Debug - work this
		ui16_throttle_accumulated += ui8_value;
		ui8_temp = ui16_throttle_accumulated >> 4; //read in value from adc
		//printf("updatereqestTrque: ui8_temp %u  \r\n",ui8_temp); ///Debug 
		...
		//printf("updatereqestTrque: #2 magnets  \r\n"); ///Debug	
		ui16_sum_torque = 0;
		...
		//printf("updatereqestTrque: #3 ui16_sum_torque /= NUMBER_OF_PAS_MAGS  \r\n"); ///Debug
