...
BitStatus checkUnderVoltageOverride(){
	...
	if (ui8_BatteryVoltage < ui8_temp) {
		...
		//printf("UNDERVOLTAGE !!!, ui32_dutycycle %u  \r\n",ui32_dutycycle); ///Debug
		return 1;
    }
	return 0;
}
...
uint16_t aca_setpoint(uint16_t ui16_time_ticks_between_pas_interrupt, uint16_t setpoint_old) {
	...
	if (((ui16_aca_flags & EXTERNAL_SPEED_SENSOR) == EXTERNAL_SPEED_SENSOR)) {
		...
		//printf("setpoint: #1 ext speed sensor"); ///Debug
	}else{
	...
	if (ui8_offroad_state == 255) {
		...
		//printf("setpoint: #2 speed limit 80 \r\n"); ///Debug
		...
		printf("setpoint: #3 speed limit <=2 \r\n"); ///Debug
		...
		printf("setpoint: #4 speed limit >2 \r\n"); ///Debug
		...
		//printf("setpoint: #5 speed limit WO PAS \r\n"); ///Debug
	} else {
		ui8_speedlimit_actual_kph = ui8_speedlimit_kph;
		printf("setpoint: #6 speed limit normal 25 \r\n"); ///Debug
	}
	...
	/*printf("setpoint: ui16_BatteryCurrent_accumulated1 %u\r\n", ui16_BatteryCurrent_accumulated);
	ui16_BatteryCurrent_accumulated -= ui16_BatteryCurrent_accumulated >> 3;
	printf("setpoint: ui16_BatteryCurrent_accumulated2 %u\r\n", ui16_BatteryCurrent_accumulated);
	ui16_BatteryCurrent_accumulated += ui16_adc_read_motor_total_current();
	ui16_BatteryCurrent = ui16_BatteryCurrent_accumulated >> 3;
	printf("setpoint: ui16_BatteryCurrent_accumulated3 %u\r\n", ui16_BatteryCurrent_accumulated); ///Debug
	printf("setpoint: ADC>>> %u\r\n", ui16_adc_read_motor_total_current()); ///Debug
	printf("setpoint: ui16_BatteryCurrent %u\r\n", ui16_BatteryCurrent); ///Debug*/
	
	
	ui16_BatteryCurrent=ui16_adc_read_motor_total_current();
	...
	if (brake_is_set()) {
		...
		if (((ui16_aca_flags & TQ_SENSOR_MODE) != TQ_SENSOR_MODE)) {
			...
			if (ui16_time_ticks_between_pas_interrupt_smoothed > ui16_s_ramp_end) {
				...
				controll_state_temp += 2;
					...
					//printf("setpoint: #10 torque sensor mode  \r\n"); ///Debug
		...
		if (((ui16_aca_flags & TQ_SENSOR_MODE) != TQ_SENSOR_MODE)) {
			if (ui8_speedlimit_kph > 1){
				...
				//printf("setpoint: #12 throttle / torquesensor override following  \r\n"); ///Debug ATTENTION
				//printf("setpoint: ui16_sum_throttle %u\r\n", ui16_sum_throttle); ///Debug
			...
			printf("setpoint: #13 momentary_throttle  \r\n"); ///Debug 
		}
		...
		if ((ui16_aca_flags & ASSIST_LVL_AFFECTS_THROTTLE) == ASSIST_LVL_AFFECTS_THROTTLE) {
			...
			controll_state_temp += 8;
			//printf("setpoint: #14 mapping  \r\n"); ///Debug
		}
		
		//printf("setpoint: float_temp0 %u\r\n", (uint32_t)float_temp); ///Debug
		float_temp = float_temp * (float) (ui16_battery_current_max_value) / 255.0 + (float) ui16_current_cal_b; //calculate current target
		
		//printf("setpoint: float_temp1 %u\r\n", (uint32_t) float_temp); ///Debug
		//printf("setpoint: i16_battery_current_max_value %u\r\n", ui16_battery_current_max_value); ///Debug
		if ((uint32_t) float_temp > uint32_current_target) {
				...
				//printf("setpoint: #15 override current target with throttle  \r\n"); ///Debug 
		...
		if (setpoint_old > 0 && (uint32_current_target - ui16_current_cal_b)*255 / setpoint_old > PHASE_CURRENT_MAX_VALUE) { // limit phase current according to Phase Current = battery current/duty cycle
			...
			//printf("setpoint: #16 phase current limiting  \r\n"); ///Debug
		...
		if ((ui16_aca_experimental_flags & DC_STATIC_ZERO) == DC_STATIC_ZERO) {
			...
			if (ui8_walk_assist) uint32_current_target = 3 + ui16_current_cal_b;
			//send current target to PI-controller
			ui32_dutycycle = PI_control(ui16_BatteryCurrent, uint32_current_target,uint_PWM_Enable);
			//printf("setpoint: #17 PI controller  \r\n"); ///Debug
